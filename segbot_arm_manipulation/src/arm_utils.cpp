...
namespace segbot_arm_manipulation {
    ...
    get_collision_boxes(std::vector<sensor_msgs::PointCloud2> obstacles) {
        ...
        for (auto &obstacle : obstacles) {
            ...
            tf_listener.waitForTransform(obstacle.header.frame_id, "base_link",
                                         ros::Time(0), ros::Duration(3.0));

            sensor_msgs::PointCloud2 object_cloud;
            pcl_ros::transformPointCloud("base_link", obstacle, object_cloud, tf_listener);
    ...
    sensor_msgs::JointState values_to_joint_state(std::vector<float> joint_values) {
        ...
        for (unsigned int i = 0; i < 8; i++) {
            js.name.push_back(Mico::jointNames[i]);
    ...
    kinova_msgs::JointAngles state_to_angles(const sensor_msgs::JointState &state) {
        ...
        as_angles.joint1 = joint_pos[Mico::jointNames[0]];
        as_angles.joint2 = joint_pos[Mico::jointNames[1]];
        as_angles.joint3 = joint_pos[Mico::jointNames[2]];
        as_angles.joint4 = joint_pos[Mico::jointNames[3]];
        as_angles.joint5 = joint_pos[Mico::jointNames[4]];
        as_angles.joint6 = joint_pos[Mico::jointNames[5]];
